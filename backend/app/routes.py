...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    user = services.UserService.authenticate_user(db, form_data.username, form_data.password)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_my_profile(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_my_profile(
    ...
    updated_profile = services.ProfileService.update_profile(db, current_user.id, profile_data)
